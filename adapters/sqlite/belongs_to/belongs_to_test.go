...
func (ts *TestSuite) TestCreate() {
	...
	ts.True(err.IsNil(), "Tentar relacionar um link a uma categoria, ambos válidos, não deveria retornar erro")
}
...
func (ts *TestSuite) TestExists() {
	...
	ts.True(err.IsNil(), "Exists, se informado uma chave válida não deveria retornar erro")
...
func (ts *TestSuite) TestNotExists() {
	...
	ts.True(err.IsNil(), "Exists, se informado uma chave inválida não deveria retornar erro")
...
func (ts *TestSuite) TestHasLinks() {
	...
	ts.True(err.IsNil(), "HasLinks, se informado uma chave válida não deveria retornar erro")
...
func (ts *TestSuite) TestHasNoLinks() {
	...
	ts.True(err.IsNil(), "HasLinks, se informado uma chave inválida não deveria retornar erro")
...
func (ts *TestSuite) TestGetLinksByCategory_Empty() {
	...
	ts.True(err.IsNil(), "Tentar recuperar links de uma categoria vazia não deveria retornar erro")
}
...
func (ts *TestSuite) TestGetLinksByCategory_Exists() {
	...
	ts.True(err.IsNil(), "Tentar recuperar links de uma categoria válida não deveria retornar erro")
	ts.Len(links, 2)
